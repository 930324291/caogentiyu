 ...
 */
public class TeamDetialActivity1 extends BaseActivity {
...
//                bean.setLikeNum(teamBean.getLikeNum() + 1);
    ...
    private void setMovePlayers() {
        ...
        for (UserBean bean : dataList) {
            if (bean.getLocationx() >= 0 && bean.getLocationy() >= 0) {
                mTopPlayer.add(bean);
            } else {
    ...
    private void initD1() {
        CommonList.mSaveCache.addAll(dataList);
        LogUtils.d(dataList.toString() + "55555555555555");
        setMovePlayers();
    }
