 ...
 * 招人详情
 */
public class RecruitPeopleDetailActivity extends BasicActivity {
    ...
    public int getTitleRes() {
        return R.string.title_recruit_people_detail;
    }


    protected void initView() {
        commentBtn.setText("发送");
        bean = (RecruitBean) getIntent().getSerializableExtra("beandata");
        String url = CommonUtils.getRurl(bean.getTeam().getIconUrl());
        ...
        timeview.setText(CommonUtils.getDateStr(bean.getOpTime(),"yyyy-MM-dd HH:mm"));
        if(bean.getContent() != null){
            contentV.setText(Html.fromHtml(bean.getContent()));
        }else{
            contentV.setText("");
        }
    ...
    @OnClick({R.id.commentBtn,R.id.imageview})
        ...
        switch (v.getId()) {
            case R.id.imageview:
                if (FootBallApplication.APPLacationRole == FootBallApplication.ROLE_TeamMember) {
                    ...
                    intent.putExtra("beanid", bean.getTeam().getId());
